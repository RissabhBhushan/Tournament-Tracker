...
namespace TrackerLibrary.DataAccess.TextHelpers
{
    public static class TextConnectorProcess
    {
        ...
        {
            //id,tournament name, entry fee, (id|id|id)entredteams, (id|id)prizes, (id^id|id^id)rounds
            List<TournamentModel> output = new List<TournamentModel>();
            List<TeamModel> teams = GlobalConfig.TeamFile.FullFilePath().LoadFile().ConvertToTeamModel();
            List<PrizeModel> prize = GlobalConfig.PrizesFile.FullFilePath().LoadFile().ConvertToPrizeModel();
        ...
        public static void SaveRoundToFile(this TournamentModel model)
        {
            ...
            // Lopp thrugh each entry get the ida nd save it
        ...
        public static List<MatchupModel> ConvertToMatchupModel(this List<string> lines)
        {
            ...
            foreach (string line in lines)
            {
                ...
                p.Id = int.Parse(col[0]);
                p.Entries = ConvertStringToMatchupEntryModel(col[1]);
                if (col[2].Length == 0)
                {
                    p.Winner = null;
                }
                ...
                {
                    p.Winner = LookupTeamById(int.Parse(col[2]));
                }
                p.MatchupRound = int.Parse(col[3]);
        ...
        private static string ConvertPrizeListToString(List<PrizeModel> prize)
        {
            ...
            foreach (PrizeModel pr in prize)
            {
                output += $"{ pr.Id }|";
            }
            output = output.Substring(0, output.Length - 1);
